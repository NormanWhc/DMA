...
def molebert(drug_list):
    ...
    for x, drug in enumerate(drug_list):
        ...
        num_tasks = 1
        model = GNN_graphpred(5, 300, num_tasks, JK="last", drop_ratio=0.5, graph_pooling="mean", gnn_type="gin")
        model.from_pretrained('./model/{}'.format('Mole-BERT.pth'))
        dataset = [data]
