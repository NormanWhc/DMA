...
def smolechemfusion(protein_list):
    ...
    feature_mole = chemberta(protein_list)

    feature_fusion = pd.concat([feature_smole,feature_mole],axis=1)
    # feature_fusion_array = feature_fusion.values
    # feature_fusion_tensor = torch.from_numpy(feature_fusion_array).float().unsqueeze(0)


    # feature_mole = torch.tensor(feature_mole.values, dtype=torch.float32, device='cuda').unsqueeze(0)
    # feature_smole = torch.tensor(feature_smole.values, dtype=torch.float32, device='cuda').unsqueeze(0)

    # features_mole_resized = torch.nn.functional.interpolate(features_mole.unsqueeze(0), size=(824, 512),
    #                                                         mode='bilinear', align_corners=False).squeeze(0)
    # y_resized = torch.nn.functional.interpolate(feature_mole, size=(512,), mode='linear', align_corners=False)
    # x_resized = torch.nn.functional.interpolate(feature_mole, size=feature_smole.shape[2],
    #                                             mode='nearest')
    # print(feature_smole.shape,feature_mole.shape)

    # os.environ['CUDA_VISIBLE_DEVICES'] = "0"
    # device = torch.device("cuda:0")

    # model = iAFF(channels=824,r=4)
    # model = model.to(device).train()
    # print(feature_smole.shape, y_resized.shape)
    # output = model(feature_smole, y_resized)
