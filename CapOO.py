...
def fitting(train_p, test_p, train_d, test_d, train_y, test_y, model_type, lr, ep, sl, path, taxonomy,dti, batchsize=64):
    ...
    if "onehot_onehot_capsule" in model_type:
        param_grid = {
            "target_dense": [200],
            "batch_size": [64],
            "seq_len": sl,
                                    ...
                                    for routings in param_grid["routings"]:
                                        for kernel_size in param_grid["kernel_size"]:
                                            ...
                                            # valid_dataset = MPNNDataset(train_p_val, train_d_val, train_y_val)
                                                ...
                                                model_1 = model_onehot_onehot_capsule(param=param)
...
if __name__ == '__main__':
    ...
    if not os.path.exists(model_name_normal + data_prefix + '_' + dti):
        os.makedirs(model_name_normal + data_prefix + dti)
    if os.path.exists(model_name_normal + data_prefix + '_' + dti + "/" + model_name_normal + "protein.txt"):
        df = open(model_name_normal + data_prefix + '_' + dti + "/" + model_name_normal + 'protein.txt', 'rb')
        protein = pickle.load(df)
        df.close()
        df = open(model_name_normal + data_prefix + '_' + dti + "/" + model_name_normal + 'drug.txt', 'rb')
        drug = pickle.load(df)
        df.close()
        df = open(model_name_normal + data_prefix + '_' + dti + "/" + model_name_normal + 'y.txt', 'rb')
        y = pickle.load(df)
        df.close()
    else:
        protein, drug, y = newdata(dti, protein_descripter, protein_sequence_length, drug_descripter, form_negative)
        fw = open(model_name_normal + data_prefix + '_' + dti + "/" + model_name_normal + "protein.txt", 'wb')
        pickle.dump(protein, fw)
        fw.close()
        fw = open(model_name_normal + data_prefix + '_' + dti + "/" + model_name_normal + "drug.txt", 'wb')
        pickle.dump(drug, fw)
        fw.close()
        fw = open(model_name_normal + data_prefix + '_' + dti + "/" + model_name_normal + "y.txt", 'wb')
        pickle.dump(y, fw)
        fw.close()
