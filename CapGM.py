...
def fitting(train_p, test_p, train_d, test_d, train_y, test_y, model_type, lr, ep, sl, path, taxonomy,dti, batchsize=64):
    ...
    if "genalm_MPNN_capsule" in model_type:
        param_grid = {
        ...
        fw.write(str(200) + "," + str(batch_size) + "," + str(
            63) + "," + str(4) + "," + str(
            8) + "," + str(512) + "," + str(
            2) + "," + str(3) + "," + str(5) + "\n")
        fw.close()
        for i, (train_index, val_index) in enumerate(cv.split(train_p)):
            ...
            model_1 = model_genalm_MPNN_capsule(
            ...
            model_1.fit(train_dataset, epochs=ep, batch_size=batchsize,
                        verbose=2,
        ...
        fw.write(str(best_score) + "," + str(best_target_dense) + "," + str(best_batch_size) + "," + str(
            best_message_units) + "," + str(best_message_steps) + "," + str(best_num_attention_heads) + "," + str(
            best_dense_units) + "," + str(best_num_capsule) + "," + str(best_routings) + "," + str(
            best_kernel_size))
        fw.close()
        model_1 = model_genalm_MPNN_capsule(
            # target_dense=best_target_dense, batch_size=best_batch_size, seq_len=sl,
        ...
        train_d0, train_d1, train_d2 = train_d
        train_d = (tf.ragged.constant(train_d0, dtype=tf.float32), tf.ragged.constant(train_d1, dtype=tf.float32),
                   tf.ragged.constant(train_d2, dtype=tf.int64))
        train_dataset_all = MPNNDataset(train_p, train_d, train_y)
        history = model_1.fit(train_dataset_all, batch_size=batchsize, epochs=ep,
...
def evaluate(model_parh, test_p, test_d, test_y, model_name):
    ...
    if "onehot_MPNN_capsule" in model_name:
        ...
        model = keras.models.load_model(model_name + "/" + model_name + ".ckpt", compile=False,
                                        custom_objects={'Capsule': Capsule, 'Length': Length,
                                                        'MessagePassing': MessagePassing,
                                                        'TransformerEncoderReadout': TransformerEncoderReadout})
    else:
