...
def fitting(train_p, test_p, train_d, test_d, train_y, test_y, model_type, lr, ep, sl, path, taxonomy,dti, batchsize=64):
    ...
    if "bert_chemberta_attention" in model_type:
        param_grid = {
            "target_dense": [200],
            "batch_size": [64],
            "seq_len": sl,
                                ...
                                for num_capsule in param_grid["num_capsule"]:
                                    for routings in param_grid["routings"]:
                                        for kernel_size in param_grid["kernel_size"]:
                                            ...
                                            fw.close()
                                            # if dti != 'data/SM2miR4.csv':
                                            #     train_p_train, train_p_val, train_d_train, train_d_val, train_y_train, train_y_val = get_test_validation(train_p, train_d, train_y, dti)
                                            ...
                                            #     model_1 = model_bert_chemberta_attention(param=param)
        ...
        fw.write(str(best_score) + "±" + str(np.std(all_acc_scores)) + "," + str(np.mean(all_specificity)) +"±"+str(np.std(all_specificity)) + str(np.mean(all_sensitivity)) +"±"+str(np.std(all_sensitivity))+ str(np.mean(all_aucroc)) +"±"+str(np.std(all_aucroc))+ str(np.mean(all_aupr)) +"±"+str(np.std(all_aupr))+ str(np.mean(all_f1)) +"±"+str(np.std(all_f1))+ "\n")
